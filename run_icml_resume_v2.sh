#!/usr/bin/env bash
# Resume script - picks up from current state
# Current state: Seed 1 PPO MuJoCo/DMC baselines done, need everything else
set -euo pipefail
...
run_atari_stress() {
  ...
  # SAC Stress: High LR (Atari) A1, A2
  run_benchmark "${FRAGILE_ATARI}" \
...
# ============================================================
# RESUME POINT: Complete seed 1, then continue
# Existing: PPO MuJoCo/DMC seed 1 (A0 only, no A1)
# ============================================================

echo "=============================================="
echo "RESUMING: Completing Seed 1 (SAC, TD3, stress)"
echo "=============================================="

# Seed 1: PPO A1 (monitor) - need to run
run_benchmark "${MUJOCO_ENVS}" \
  "python cleanrl/ppo_continuous_action_envpool.py --rsubt-monitor --num-envs ${NUM_ENVS_PPO} --num-steps ${NUM_STEPS_PPO} --total-timesteps ${MUJOCO_STEPS} --no-anneal-lr --learning-rate 3e-4" \
  1
run_benchmark "${DMC_ENVS}" \

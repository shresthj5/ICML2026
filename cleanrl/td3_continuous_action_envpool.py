# EnvPool-native TD3 for continuous action spaces (MuJoCo + DMC)
...
class Args:
    ...
    """the learning rate of the optimizer (actor + critic)"""
...
if __name__ == "__main__":
    ...
    actor_target = Actor(obs_dim, action_dim, action_scale, action_bias).to(device)
    ...
    for global_step in range(args.total_timesteps):
        # Action logic with exploration noise
        if global_step < args.learning_starts:
            ...
            with torch.no_grad():
                ...
                noise = np.random.normal(0, action_scale * current_exploration_noise, size=actions.shape).astype(np.float32)
        ...
        if is_dmc:
            ...
            dones = timestep.last().astype(np.float32)
        else:
            ...
            dones = split.terminations.astype(np.float32)
        ...
        if global_step > args.learning_starts:
            ...
            with torch.no_grad():
                ...
                clipped_noise = (torch.randn_like(b_actions, device=device) * args.policy_noise).clamp(
                    -args.noise_clip, args.noise_clip
                ) * actor_target.action_scale
                min_a = torch.as_tensor(action_low, device=device)
                max_a = torch.as_tensor(action_high, device=device)
                next_state_actions = actor_target(b_next_obs) + clipped_noise
                next_state_actions = torch.max(torch.min(next_state_actions, max_a), min_a)
        ...
        if args.eval_interval_steps > 0 and (global_step - last_eval_step) >= args.eval_interval_steps:
            eval_stats = evaluate_agent(actor, envpool_id, backend, args.eval_episodes, args.eval_seed, device)
